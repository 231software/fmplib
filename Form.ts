...
//所有的表单都不能改异步
//因为从表单的用途来看，表单结束时会有多个状态，每个状态都很可能对应着完全不同的行为
//不像http那边，返回的时候返回的数据是什么用途的都有，至于出错则是直接按错误处理，很直观
//如果表单这边想做那种更简单的，对关闭按钮有返回或丢弃之类的固定处理方法，然后简单表单也是从列表中选择的话
//就可以用户自己在外面包一层，然后去简化流程
//从表单的复杂用途来看满月平台直接给提供这种的大可不必
